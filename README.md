...
為了提供使用者一個「**如行雲流水般輕快、安裝與反安裝生命週期清晰受控**」的良好體驗，我們將核心獨立分流並進行全面深度重構，正式立項為全新的獨立專案 —— **「行雲_繁-A」（floating_cloud）**。

---

## 重大修改歷程 (Changelog & Architecture Evolution)

### 0. v1.2.0 版本更新說明（穩定性、卸載防護與系統邊界文檔）
- **TIS 啟用防禦（冪等性守衛）**：`InputSourceHelper` 與安裝腳本在調用 `TISEnableInputSource` 前先檢查啟用狀態，避免重複呼叫造成非預期的狀態回傳或系統輸入法清單抖動。
- **卸載前輸入源平順轉移**：反安裝腳本在執行初期若偵測到當前輸入法正為「行雲_繁-A」，會自動切換至系統其他可用輸入法（如 ABC），解除使用中鎖定，保障檔案刪除流程平順完整。
- **系統偏好精準比對與移除確認**：改採確切 Bundle ID（`com.vader.inputmethod.XingYunIME`）比對，避免模糊比對；反安裝加入互動確認與完成提示，防止誤觸。
- **詳實客觀的系統安全邊界指引**：新增 [《安裝與卸載標準操作指南》](doc/INSTALL_AND_UNINSTALL_GUIDE.md)，完整剖析 macOS TCC / SIP（`com.apple.macl`）對輸入法偏好清單的保護機制，實事求是地說明反安裝後若 macOS「系統設定」介面留有名稱快取，使用者手動按「—」號即可完成清理的原理。
- **DMG 封裝手冊更新**：打包腳本產出之磁碟映像檔隨附最新操作說明，提供清晰客觀的操作與清理須知。
...
> 完整系統狀態架構、驗證邊界與 12 項驗收標準請參閱：[《行雲_繁-A 安裝、輸入來源註冊與系統狀態說明》](doc/SYSTEM_STATE_AND_INSTALLATION.md) 與 [《安裝與卸載標準操作指南》](doc/INSTALL_AND_UNINSTALL_GUIDE.md)
...
- **安全反安裝機制與系統防護邊界**：
  `反安裝行雲_繁-A.command` 在卸載時會先主動將作用中輸入法切換至其他輸入來源（如 ABC），安全結束背景服務並從 LaunchServices 註銷 App 記錄，隨後徹底刪除實體檔案（`~/Library/Input Methods/行雲_繁-A.app`）。同時嚴格遵循 macOS 系統邊界，不使用破壞性指令誤傷其他輸入法。詳細系統機制與說明請見 [《安裝與卸載標準操作指南》](doc/INSTALL_AND_UNINSTALL_GUIDE.md)。
...
1. 雙擊執行「**反安裝行雲_繁-A.command**」：
   - 程式將自動終止背景服務、自 LaunchServices 註銷核心索引、撤銷 TIS 輸入法啟用，並徹底刪除實體檔案（`~/Library/Input Methods/行雲_繁-A.app`）。
2. **macOS 系統設定「文字輸入方式」面板之最後清理（重要限制說明）**：
   - **蘋果系統安全設計（TCC / SIP 防護）**：macOS 為了防止惡意軟體私自竄改或刪除使用者的鍵盤設定，對 `com.apple.inputsources` 使用者偏好清單實施強制沙盒保護，**系統嚴格禁止任何第三方指令或反安裝腳本（包含 root/sudo）在背景私自竄改該介面清單**。
   - **最後清理步驟**：若執行反安裝後，macOS「系統設定」>「鍵盤」>「文字輸入方式」（編輯）列表中仍顯示「行雲_繁-A」歷史圖示，只需**點選該項目並按左下角的「—」（減號）**即可徹底清除；因 App 本體已刪除，未來在按「+」新增時也不會再出現。
